...
export default {
  ...
  computed: {
    currentModelSrc() {
      return Number(this.form.gender) === 2
        ? `/models/profile/girl.glb?v=${MODEL_VERSION}`
        : `/models/profile/boy.glb?v=${MODEL_VERSION}`;
    },
    displayName() {
  ...
  watch: {
    currentModelSrc(newValue, oldValue) {
...
.profile-avatar-upload {
  ...
  border-radius: 20px;
  background: linear-gradient(180deg, rgba(255, 255, 255, 0.76), rgba(244, 250, 247, 0.92));
  box-shadow:
    inset 0 1px 0 rgba(255, 255, 255, 0.82),
    0 10px 24px rgba(28, 94, 81, 0.08);
  text-align: center;
...
.profile-avatar-upload ::v-deep .room_add_img {
  ...
  margin-left: 20px;
  border-radius: 20px;
  background: linear-gradient(180deg, rgba(237, 246, 242, 0.92), rgba(231, 241, 237, 0.96));
  box-shadow: inset 0 0 0 1px rgba(131, 173, 164, 0.16);
}
...
.profile-avatar-upload ::v-deep .room_add_img img {
  width: 34px;
  height: 34px;
  opacity: 0.56;
}
...
.profile-avatar-upload ::v-deep .room_img img {
  object-fit: cover;
}
...
.profile-page ::v-deep .el-tabs__header {
  margin-bottom: 12px;
}
